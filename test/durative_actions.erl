...
non_duration_var_in_constraint_results_in_error_test() ->
    DomainStr ="(define (domain foo) (:action bar :duration (<= ?asdf 300))",
    try
...
start_effect_test() ->
    DomainStr = "(define (domain foo) (:action bar :duration (<= ?duration 300) :effect (at start ())))",
    Domain = epddl:parse(DomainStr),
...
stop_effect_test() ->
    DomainStr = "(define (domain foo) (:action bar :duration (<= ?duration 300) :effect (at end ())))",
    Domain = epddl:parse(DomainStr),

    [Bar] = Domain#domain.actions,
    Effect = Bar#action.effect,
...
nested_durative_effect_test() ->
    DomainStr = "(define (domain foo) (:action bar :duration (<= ?duration 300) :effect (and (at end ()) (at start ()))))",
    Domain = epddl:parse(DomainStr),
